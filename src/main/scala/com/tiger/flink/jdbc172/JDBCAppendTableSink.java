...
package com.tiger.flink.jdbc172;
 ...
 * check pointing is enabled). However, one common use case is to run idempotent queries
...
public class JDBCAppendTableSink implements AppendStreamTableSink<Row>, BatchTableSink<Row> {
	...
	public TableSink<Row> configure(String[] fieldNames, TypeInformation<?>[] fieldTypes) {
		...
		String sinkSchema =
				IntStream.of(types).mapToObj(JDBCTypeUtil::getTypeName).collect(Collectors.joining(", "));
		String tableSchema =
				Stream.of(fieldTypes).map(JDBCTypeUtil::getTypeName).collect(Collectors.joining(", "));
